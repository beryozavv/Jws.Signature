using System.Security.Cryptography;
using Jws.Signature.KeyExtractor;
using Jws.Signature.Signing;
using Xunit;
using Xunit.Abstractions;

namespace Jws.Signature.Tests;

public class SigningIntegrationTests
{
    private readonly ITestOutputHelper _testOutputHelper;

    public SigningIntegrationTests(ITestOutputHelper testOutputHelper)
    {
        _testOutputHelper = testOutputHelper;
    }

    [Fact]
    public void GenRsa()
    {
        var rsa = RSA.Create();

        rsa.KeySize = 4096;

        // Private key export.
        string hdrPrv = "-----BEGIN RSA PRIVATE KEY-----";
        string ftrPrv = "-----END RSA PRIVATE KEY-----";
        string keyPrv = Convert.ToBase64String(rsa.ExportRSAPrivateKey());
        string pemPrv = $"{hdrPrv}\n{keyPrv}\n{ftrPrv}"; //concatenate header and footer to actual key

        _testOutputHelper.WriteLine(pemPrv);

        // Public key export (with a correction for more accuracy from RobSiklos's comment to have the key in PKCS#1 RSAPublicKey format)
        string hdrPub = "-----BEGIN RSA PUBLIC KEY-----";
        string ftrPub = "-----END RSA PUBLIC KEY-----";
        string keyPub = Convert.ToBase64String(rsa.ExportRSAPublicKey());
        string pemPub = $"{hdrPub}\n{keyPub}\n{ftrPub}"; //conc

        _testOutputHelper.WriteLine(pemPub);

// -----BEGIN RSA PRIVATE KEY-----
// 
// -----END RSA PRIVATE KEY-----
// -----BEGIN RSA PUBLIC KEY-----
// 
// -----END RSA PUBLIC KEY-----

    }

    [Fact]
    public void TestSign()
    {
        var signDataRsaService = new SignDataRsaService(new DefaultPrivateKeyExtractor());

        var data = "eyJhbGciOiJSUzI1NiJ9.eyJJZCI6MSwiTmFtZSI6InRlc3QgZW50aXR5IDEiLCJEZXNjcmlwdGlvbiI6ImZpcnN0IHRlc3QifQ==";
        var signature = signDataRsaService.SignData(data);

        var verifySignService = new VerifySignService(new DefaultPublicKeyExtractor());

        var verifySign = verifySignService.VerifySign(data, signature);
        
        Assert.True(verifySign);
        
        _testOutputHelper.WriteLine("Verifying result = "+verifySign);
    }
    
    [Fact]
    public void TestJwsSign()
    {
        var signDataRsaService = new SignDataRsaService(new DefaultPrivateKeyExtractor());

        var data = "eyJhbGciOiJSUzI1NiJ9.eyJJZCI6MSwiTmFtZSI6InRlc3QgZW50aXR5IDEiLCJEZXNjcmlwdGlvbiI6ImZpcnN0IHRlc3QifQ==";
        var signature = signDataRsaService.SignData(data);

        var verifySignService = new VerifySignService(new DefaultPublicKeyExtractor());

        var verifySign = verifySignService.VerifySign(data, signature);
        
        Assert.True(verifySign);
        
        _testOutputHelper.WriteLine("Verifying result = "+verifySign);
    }
}